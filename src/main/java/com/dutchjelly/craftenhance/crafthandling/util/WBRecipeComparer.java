...
public class WBRecipeComparer {
    ...
    private static RecipeItem[] ensureNoGaps(final RecipeItem[] items) {
        return Arrays.asList(items).stream().filter(x -> x != null && x.getItem() != null && x.getItem().getType() != Material.AIR).toArray(RecipeItem[]::new);
    }
    ...
    public static boolean ingredientsMatch(ItemStack[] a, RecipeItem[] b, final IMatcher<ItemStack> matcher) {
        //array with all values to false.
        a = ensureNoGaps(a);
        b = ensureNoGaps(b);
